...
class ModelMonitoringService:
    ...
    async def _monitor_performance(self, model_name: str, model_version: str,
                                  predictions: np.ndarray):
    ...
    async def _log_inference(self, model_name: str, model_version: str,
                            input_data: pd.DataFrame, predictions: np.ndarray,
                            latency: float, quality_issues: List[Dict]):
        ...
        try:
            self.es_client.index(
                index=f"ml-inference-{datetime.now().strftime('%Y.%m')}",
                body=log_entry
            )
        except Exception as e:
    ...
    def _create_panel(self, title: str, query: str, panel_type: str,
...
class PerformanceProfiler:
    ...
    def get_profile_summary(self, model_name: str) -> Dict:
        ...
        model_profiles = [p for k, p in self.profiles.items() if k.startswith(model_name)]
...
class LogAggregator:
    ...
    async def aggregate_logs(self, model_name: str,
        ...
        return {
            ...
            'latency_percentiles': result['aggregations']['latency_percentiles']['values'],
...
def setup_monitoring(config: MonitoringConfig) -> Tuple[ModelMonitoringService, AlertingService]:
