...
request_latency = Histogram('ml_api_request_latency_seconds', 'Request latency', ['endpoint'])
prediction_count = Counter('ml_api_predictions_total', 'Total predictions made', ['model'])
model_version_gauge = Gauge('ml_api_model_version', 'Current model version', ['model_name'])
...
class ModelManager:
    ...
    async def load_model(self, model_name: str, model_version: str = "latest"):
        ...
        try:
            ...
            model_version_gauge.labels(model_name=model_name).set(
                float(model_version.replace("v", "") if model_version != "latest" else 0)
            )
    ...
    async def explain_prediction(self, features: Dict, model_name: str, model_version: str):
