...
1. Identify which modules/symbols are public using `docs/DOCS_STYLE.md`.
...
3. Update docstrings for any public symbol you touch, and add `Examples`
   sections for the usage-heavy APIs recorded in `docs/DOC_COVERAGE.md`.

## 2. Writing Docstrings

- Follow the Google-style template (summary → Args → Returns → Raises →
  Side Effects → Complexity → Examples). See `docs/DOCS_STYLE.md` for details.
...
- [ ] Top 20 APIs listed in `docs/DOC_COVERAGE.md` contain `Examples`.
- [ ] Inline comments explain intent/guardrails for complex logic.
- [ ] Type hints, Raises, and docstrings stay in sync.
- [ ] `docs/DOC_COVERAGE.md` describes what changed plus remaining gaps.
