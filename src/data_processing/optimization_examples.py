...
def example_parallel_outlier_detection():
    ...
    df_clean = processor.parallel_outlier_detection(
        df, columns=metric_cols, method="zscore", threshold=3.0
    )
...
def example_optimized_cuped():
    ...
    df["post_experiment_value"] = df["pre_experiment_value"] * 0.7 + np.random.normal(
        30, 10, n_users
    )
...
def example_memory_efficient_aggregation():
    ...
    result = processor.memory_efficient_groupby(
        df, group_col="group_id", agg_funcs=agg_funcs
    )
...
def example_complete_pipeline():
    ...
    # Add treatment effect
    treatment_mask = df["group"] == "treatment"
    df.loc[treatment_mask, "experiment_purchases"] = np.random.poisson(
        4, treatment_mask.sum()
    )
    ...
    treatment_purchases = results.loc[
        "treatment", ("experiment_purchases_cuped", "mean")
    ]
