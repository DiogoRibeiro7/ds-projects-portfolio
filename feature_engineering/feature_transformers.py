...
class DimensionalityReducer(BaseEstimator, TransformerMixin):
    ...
    def _create_autoencoder(self, input_dim: int):
        ...
        try:
            ...
            encoded = Dense(
                max(input_dim // 4, self.n_components * 2), activation="relu"
            )(encoded)
            encoded = BatchNormalization()(encoded)
            ...
            decoded = Dense(
                max(input_dim // 2, self.n_components * 4), activation="relu"
            )(decoded)
            decoded = BatchNormalization()(decoded)
...
class ClusteringTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self.method == "kmeans":
            self.clusterer = KMeans(
                n_clusters=self.n_clusters, random_state=42, **self.kwargs
            )
            ...
            self.clusterer = GaussianMixture(
                n_components=self.n_clusters, random_state=42, **self.kwargs
            )
    ...
    def transform(self, X):
        ...
        if self.method == "gmm":
            ...
            labels = (
                self.clusterer.predict(X)
                if hasattr(self.clusterer, "predict")
                else self.clusterer.labels_
            )
...
class InteractionTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_df = pd.DataFrame(X) if not isinstance(X, pd.DataFrame) else X
        result = (
            X_df.copy() if not self.interaction_only else pd.DataFrame(index=X_df.index)
        )
            ...
            if col in self.discretizers:
                ...
                if self.encode in ["onehot", "onehot-dense"] or binned.shape[1] > 1:
                    ...
                    binned_df = pd.DataFrame(
                        binned, columns=col_names, index=X_df.index
                    )
                else:
                    binned_df = pd.DataFrame(
                        binned, columns=[f"{col}_binned"], index=X_df.index
                    )
...
class TargetTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, y):
        ...
        if self.method == "log":
            if (y <= 0).any():
                print(
                    "Warning: Negative values found. Using log1p with shifted values."
                )
        ...
        if "rolling" in self.augmentation_types:
            ...
            for window in [3, 5, 7]:
                ...
                rolling_mean.columns = [
                    f"{col}_roll_mean_{window}" for col in X_df.columns
                ]
                augmented_dfs.append(rolling_mean)

                rolling_std = X_df.rolling(window, min_periods=1).std()
                rolling_std.columns = [
                    f"{col}_roll_std_{window}" for col in X_df.columns
                ]
