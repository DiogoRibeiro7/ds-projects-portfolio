...
Examples:
  ...
  python run_notebook_tests.py --timeout 300
    ...
    parser.add_argument(
        "--parallel", action="store_true", help="Run notebook tests in parallel"
    )
    ...
    parser.add_argument(
        "--report-only", help="Generate HTML report from existing JSON results file"
    )

    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")

    parser.add_argument(
        "--fail-fast", action="store_true", help="Stop testing after first failure"
    )
    ...
    for status, count in summary["status_counts"].items():
        ...
        status_display = (
            status.replace("_", " ").title() if isinstance(status, str) else str(status)
        )
    ...
    if args.html:
        html_path = (
            Path(args.output)
            / f"notebook_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        )
...
def run_validation_only(notebooks, args):
    ...
    for i, notebook_path in enumerate(notebooks, 1):
        ...
        try:
            ...
            summary["status_counts"]["error"] = (
                summary["status_counts"].get("error", 0) + 1
            )
    ...
    # Calculate pass rate
    passed = summary["status_counts"].get("passed", 0) + summary["status_counts"].get(
