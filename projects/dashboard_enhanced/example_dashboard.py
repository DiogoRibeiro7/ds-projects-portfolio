...
class PortfolioDashboard:
    ...
    def _setup_data_sources(self):
        ...
        def get_realtime_metrics(filters=None):
            ...
            time_range = pd.date_range(end=current_time, periods=100, freq="1min")

            data = pd.DataFrame(
                {
                    "timestamp": time_range,
                    "cpu_usage": np.random.uniform(20, 80, 100)
                    + np.sin(np.linspace(0, 4 * np.pi, 100)) * 10,
                    "memory_usage": np.random.uniform(30, 70, 100)
                    + np.cos(np.linspace(0, 4 * np.pi, 100)) * 5,
            ...
            [
                ...
                dbc.NavbarSimple(
                    children=[
                        ...
                        dbc.NavItem(
                            dbc.NavLink("ML Models", href="/models", active="exact")
                        ),
                        dbc.NavItem(
                            dbc.NavLink("Analytics", href="/analytics", active="exact")
                        ),
                        dbc.NavItem(
                            dbc.NavLink("Real-time", href="/realtime", active="exact")
                        ),
                        dbc.DropdownMenu(
                            children=[
                                dbc.DropdownMenuItem("Export PDF", id="export-pdf-btn"),
                                dbc.DropdownMenuItem(
                                    "Export PowerPoint", id="export-pptx-btn"
                                ),
                                dbc.DropdownMenuItem(
                                    "Export Data", id="export-data-btn"
                                ),
                                dbc.DropdownMenuItem(divider=True),
                                dbc.DropdownMenuItem(
                                    "API Docs", href="/api/docs", external_link=True
                                ),
                            ],
                            nav=True,
                            in_navbar=True,
                ...
                dcc.Interval(
                    id="interval-fast", interval=1000, n_intervals=0
                ),  # 1 second
                dcc.Interval(
                    id="interval-medium", interval=5000, n_intervals=0
                ),  # 5 seconds
                dcc.Interval(
                    id="interval-slow", interval=30000, n_intervals=0
                ),  # 30 seconds
                # Store components
    ...
    def _setup_custom_callbacks(self):
        ...
        # Dark mode toggle
        @self.dashboard.app.callback(
            [Output("main-container", "style"), Output("theme-store", "data")],
        ...
        def handle_export(pdf_clicks, pptx_clicks, data_clicks):
            ...
            if button_id == "export-pdf-btn":
                ...
                figures = self._collect_current_figures()
                filepath = self.dashboard.export_to_powerpoint(
                    figures, "dashboard_export.pptx"
                )
                return dbc.Alert(
    ...
    def _create_overview_page(self):
        ...
        funnel_fig = px.funnel(
            funnel_data, x="count", y="stage", title="Conversion Funnel"
        )
        ...
        return html.Div(
            [
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            [dcc.Graph(figure=revenue_fig, id="revenue-chart")], md=8
                        ),
                        dbc.Col(
                            [dcc.Graph(figure=segments_fig, id="segments-chart")], md=4
                        ),
                    ]
                ),
                dbc.Row(
                    [dbc.Col([dcc.Graph(figure=funnel_fig, id="funnel-chart")], md=12)]
                ),
            ]
        )
    ...
    def _create_models_page(self):
        ...
        return html.Div(
            [
                ...
                dbc.Row(
                    [
                        dbc.Col([dcc.Graph(figure=radar_fig, id="radar-chart")], md=6),
                        dbc.Col([dcc.Graph(figure=time_fig, id="time-chart")], md=6),
                    ]
                ),
                dbc.Row(
                    [
                        dbc.Col(
                            [dcc.Graph(figure=heatmap_fig, id="heatmap-chart")], md=12
                        )
                    ]
                ),
            ]
        ...
        return html.Div(
            [
                ...
                dbc.Row(
                    [
                        dbc.Col(
                            [dcc.Graph(figure=animated_fig, id="animated-chart")], md=12
                        )
                    ]
                ),
                dbc.Row(
                    [
                        dbc.Col(
                            [dcc.Graph(figure=scatter_3d_fig, id="3d-scatter")], md=6
                        ),
                        dbc.Col(
                            [dcc.Graph(figure=parallel_fig, id="parallel-coords")], md=6
                        ),
                    ]
                ),
            ]
                            ...
                            [
                                dbc.Card(
                                    [
                                        ...
                                        dbc.CardBody([html.Div(id="live-stats")]),
                                    ]
                                )
                            ],
                            md=4,
    ...
    def setup_realtime_callbacks(self):
        ...
        @self.dashboard.app.callback(
            Output("live-stats", "children"), [Input("interval-medium", "n_intervals")]
        )
        def update_live_stats(n):
            ...
            stats = html.Div(
                [
                    ...
                    html.Hr(),
                    html.Small(
                        f"Updated: {latest_row['timestamp'].strftime('%H:%M:%S')}"
                    ),
                ]
            )
        ...
        @self.dashboard.app.callback(
            Output("activity-feed", "children"), [Input("interval-slow", "n_intervals")]
        )
        def update_activity_feed(n):
            ...
            feed_items = [
                html.Div(
                    [html.P(activity, className="mb-1"), html.Hr(className="my-1")]
                )
    ...
    def _collect_current_figures(self):
        ...
        kpis = self.dashboard.get_data("business_kpis")
        revenue_fig = px.area(kpis, x="date", y="revenue", title="Revenue Trend")
        figures.append(revenue_fig)

        models = self.dashboard.get_data("model_performance")
        model_fig = px.bar(
            models, x="model", y="accuracy", title="Model Accuracy Comparison"
        )
