# Development Guide

## Branch Workflow

1. Fork the repository and create a feature branch:

   ```bash
   git checkout -b feature/my-change
   ```

2. Activate the virtual environment created during installation.
3. Keep commits focused (docs, linting, runtime changes should be separated).

## Tooling

| Task           | Command                                  |
| -------------- | ---------------------------------------- |
| Format         | `ruff format src tests scripts`          |
| Lint           | `ruff check src tests scripts`           |
| Static typing  | `mypy src/ --ignore-missing-imports`     |
| Unit tests     | `pytest tests/unit --maxfail=1`          |
| Docs           | `(cd docs && make html)`                 |

> NOTE: Full `mypy` coverage across production packages is still blocked by legacy type debt and missing third-party stubs. Use Python 3.11 for local type-checking:
>
> ```bash
> py -3.11 -m mypy src --python-version 3.11 --ignore-missing-imports
> ```
>
> The CI type-check job is configured to run in Python 3.11. When the remaining typing gaps are addressed, this command can be tightened to remove `--ignore-missing-imports`.

CI runs every target above (see `.github/workflows/ci.yml`), so matching the
matrix locally is the fastest path to a green pull request.

## Reproducible environment snapshots

From a clean environment, capture the exact dependency set before major edits:

```bash
# Conda (environment name: ds-portfolio)
conda env export --name ds-portfolio > environment-export.yml

# uv lockfile (prefer for deterministic Python package resolution)
uv pip compile requirements-fixed.txt -o requirements.lock

# pip frozen requirements for downstream consumers
python -m pip freeze > requirements-freeze.txt
```

To restore from lock/repro files later:

```bash
# uv
uv pip sync requirements.lock

# conda
conda env create -f environment-export.yml

# pip
python -m pip install -r requirements.lock
```

Prefer committing only one canonical snapshot file (`requirements.lock`) for
automation and keeping human-readable environment files (`environment-export.yml`)
for platform-specific reproduction.

## Documentation Workflow

- Primary authoring format is Markdown (MyST) with Sphinx + Read the Docs theme.
- Docstrings follow Google style per `DOCS_STYLE.md` and are reviewed when
  public APIs, tutorials, or reusable project code change.
- API pages under `docs/api/` are curated reviewer guides. Keep docstrings
  accurate for source readability, but do not add broad autogenerated reference
  pages unless the import path is stable in CI.

## Generated artifacts policy

- Do not commit runtime-generated outputs that are regenerated locally:
  - `docs/_build/`
  - `test_results/`
  - `outputs/`, `results/`, `logs/`
  - `.pytest_cache/`, `.mypy_cache/`, `.venv/`, `__pycache__/`
  - notebook runtime caches (`.ipynb_checkpoints/`, `notebooks/artifacts/`, `notebooks/mlruns/`)
- Keep in version control only intentionally curated artifacts used by the
  portfolio narrative (source markdown, notebooks, and curated summaries).
- If a generated artifact must be preserved, move it to a separate archive
  repository and keep only the minimum representative evidence there.
- Versioned generated artifacts are allowed only when the files are part of a
  defined narrative deliverable (for example, benchmark outputs that are
  directly referenced by project writeups).

## Local Debug Recipes

### Sample Data Cache

```bash
python scripts/fix_notebook_issues.py --download-datasets
python scripts/run_notebook_tests.py --dirs projects/ab_testing
```

### Dashboard

```bash
export FLASK_ENV=development
python -m dashboard_enhanced.app
```

### ML API

```bash
uvicorn src.api.ml_api:app --reload --host 0.0.0.0 --port 8000
```

The API exposes metrics at `/metrics`, health at `/health`, and accepts batch
payloads under `/predict/batch`.
