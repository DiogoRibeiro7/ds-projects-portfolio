...
class TestForecastModel:
    ...
    def test_lstm_forecast(self, forecast_model, train_data):
        ...
        with patch('tensorflow.keras.models.Sequential') as mock_model:
            lstm_model = forecast_model.create_lstm_model(
                input_shape=(10, 1),
                units=50
            )
...
class TestSeasonalDecomposer:
    ...
    def seasonal_data(self):
        ...
        return pd.Series(trend + seasonal + noise,
                        index=pd.date_range('2020-01-01', periods=len(t)))
    ...
    def test_multiple_seasonality(self, decomposer):
        ...
        components = decomposer.decompose_multiple_seasonality(
            data,
            periods=[1, 7]
        )
