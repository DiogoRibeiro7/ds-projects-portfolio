...
class TestFeatureEngineer:
    ...
    def test_interaction_features(self, small_dataframe):
        ...
        interactions = engineer.create_interaction_features(
            small_dataframe, cols=["a", "b"]
        )

        assert "a_x_b" in interactions.columns
        assert (
            interactions["a_x_b"] == small_dataframe["a"] * small_dataframe["b"]
        ).all()
...
class TestProductionPipeline:
    ...
    def test_initialization(self, trained_model):
        ...
        pipeline = ProductionPipeline(model=model, feature_names=list(X_test.columns))
    ...
    def test_model_versioning(self, trained_model, temp_dir):
        ...
        pipeline = ProductionPipeline(
            model=model, feature_names=list(X_test.columns), version="1.0.0"
        )
    ...
    def test_monitoring_metrics(self, trained_model):
        ...
        pipeline = ProductionPipeline(
            model=model, feature_names=list(X_test.columns), enable_monitoring=True
        )
