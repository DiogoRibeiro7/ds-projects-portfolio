...
from notebook_validator import NotebookValidator, DataValidator


class NotebookTestRunner:
    """Runs notebook tests and collects execution metrics."""

    def __init__(self, notebook_dirs: List[str], output_dir: str = "test_results",
                 timeout: int = 600, parallel: bool = False):
        """
    ...
    def _run_parallel(self, notebooks: List[Path]) -> List[Dict[str, Any]]:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(self.test_notebook, nb): nb
                      for nb in notebooks}
    ...
    def _execute_notebook(self, notebook_path: Path) -> Dict[str, Any]:
        ...
        output_notebook = Path(temp_dir) / f"test_{notebook_path.stem}_{int(time.time())}.ipynb"
    ...
    def _extract_cell_timings(self, notebook: nbformat.NotebookNode) -> List[Dict[str, Any]]:
        ...
        for i, cell in enumerate(notebook.cells):
            if cell.cell_type == 'code':
                ...
                if 'execution' in metadata:
                    ...
                    if 'iopub.execute_input' in execution_data and 'shell.execute_reply' in execution_data:
                        try:
    ...
    def _collect_metrics(self, notebook_path: Path, execution_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            with open(notebook_path, 'r', encoding='utf-8') as f:
                ...
                if execution_result['success'] and metrics['execution_time'] > 0:
                    metrics['cells_per_second'] = metrics['code_cell_count'] / metrics['execution_time']
                    metrics['mb_per_cell'] = metrics['memory_delta_mb'] / max(1, metrics['code_cell_count'])
    ...
    def _print_result(self, result: Dict[str, Any]):
        ...
        print(f"  {color}{symbol}{colors['reset']} {name:40} "
    ...
    def _write_markdown_summary(self, filepath: Path, summary: Dict[str, Any],
        ...
        with open(filepath, 'w') as f:
            ...
            f.write("| Notebook | Status | Execution Time | Validation Score | Issues |\n")
            f.write("|----------|--------|----------------|------------------|--------|\n")
            ...
            for result in results:
                ...
                f.write(f"| {name} | {status} | {exec_time:.2f}s | {score:.0f}/100 | {issues} |\n")
...
class NotebookTestConfig:
    ...
    def get_metric_ranges(self, notebook_name: str) -> Optional[Dict[str, Tuple[float, float]]]:
        """Get expected metric ranges for a notebook."""
        return self.config['metric_ranges'].get(notebook_name)


def run_notebook_tests(notebook_dirs: List[str], output_dir: str = "test_results",
                       config_path: Optional[str] = None, parallel: bool = False) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("directories", nargs="+", help="Directories containing notebooks")
