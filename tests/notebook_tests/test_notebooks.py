...
class TestNotebooks:
    ...
    @pytest.mark.parametrize("notebook_path", [
        nb for nb in NotebookTestRunner(NOTEBOOK_DIRS, str(OUTPUT_DIR)).find_notebooks()
    ])
    def test_notebook_validation(self, notebook_path):
        ...
        if not results['checks']['hardcoded_paths']['passed']:
            pytest.skip(f"Hardcoded paths found (warning): {results['checks']['hardcoded_paths']['count']}")
    ...
    def test_notebook_execution(self, notebook_path):
        ...
        # Check memory usage
        memory_delta = result.get('execution', {}).get('memory_usage', {}).get('delta_mb', 0)
        max_memory = self.config.config.get('memory_limits', {}).get('max_memory_mb', 4096)
...
def test_average_execution_time(notebook_results):
    ...
    assert avg_time < 300, \
        f"Average execution time {avg_time}s exceeds 5 minutes"


# Pytest markers for selective testing
pytest.mark.validation = pytest.mark.mark(
    pytest.mark.validation,
    "mark test as validation only (no execution)"
)

pytest.mark.execution = pytest.mark.mark(
    pytest.mark.execution,
    "mark test as execution test"
)

pytest.mark.slow = pytest.mark.mark(
    pytest.mark.slow,
    "mark test as slow (notebook execution)"
)
