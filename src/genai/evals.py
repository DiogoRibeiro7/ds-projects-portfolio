...
def hit_at_k(
    preds: list[Prediction], records: list[EvalRecord], k: int
) -> EvalMetricResult:
    per = []
    for p, r in zip(preds, records, strict=True):
...
def mrr_at_k(
    preds: list[Prediction], records: list[EvalRecord], k: int = 10
) -> EvalMetricResult:
    per = []
    for p, r in zip(preds, records, strict=True):
        if not r.gold_doc_ids:
...
def ndcg_at_k(
    preds: list[Prediction], records: list[EvalRecord], k: int = 5
) -> EvalMetricResult:
    per = []
    for p, r in zip(preds, records, strict=True):
        if not r.gold_doc_ids:
...
def context_precision(
    preds: list[Prediction], judge: LLMClient, k: int = 5
) -> EvalMetricResult:
    ...
    for p in preds:
        ...
        for r in p.retrieved[:k]:
            v = _judge_call(
                judge, "judge.context_precision", question=p.query, passage=r.text
            )
        ...
        if not rels or sum(rels) == 0:
            ...
            continue
        numerator = sum(
            (sum(rels[: i + 1]) / (i + 1)) * rels[i] for i in range(len(rels))
        )
...
def build_eval_report(
    ...
    *,
    ...
    latency_summary = {
        f"{r}_p50_ms": float(np.median(v)) if v else 0.0
        for r, v in latency_by_retriever.items()
    }
    latency_summary.update(
        {
            f"{r}_p95_ms": float(np.quantile(v, 0.95)) if v else 0.0
            for r, v in latency_by_retriever.items()
        }
    )
    cost_summary = {
    ...
    if "faithfulness" in generation:
        checks.append(
            (
                "faithfulness",
                generation["faithfulness"].mean,
                gate["min_faithfulness"],
                ">=",
            )
        )
    if "answer_relevance" in generation:
        checks.append(
            (
                "answer_relevance",
                generation["answer_relevance"].mean,
    ...
    checks.append(
        (
            "p95_latency_ms",
            latency_summary.get(f"{primary}_p95_ms", 0.0),
