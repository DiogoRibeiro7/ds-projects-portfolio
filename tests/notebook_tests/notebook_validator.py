...
class NotebookValidator:
                ...
                for pattern in path_patterns:
                    ...
                    if matches:
                        for match in matches:
                            ...
                            if not any(skip in str(match) for skip in ['./data', '../data', 'data/', '"data/', "'data/"]):
                                hardcoded_paths.append({
                                    'cell': cell_num + 1,
                                    'path': match,
                                    'line': source[:100] + '...' if len(source) > 100 else source
                                })
            ...
            if cell.cell_type == 'code':
                ...
                for lib, patterns in viz_patterns.items():
                    for pattern in patterns:
                        if re.search(pattern, source):
                            if 'show' in pattern or 'save' in pattern or 'interactive' in pattern:
                                has_show = True
                            else:
                                has_plot = True

                # Check for common plotting functions without show
                plot_commands = ['plt.plot', 'plt.scatter', 'plt.bar', 'plt.hist',
                               'sns.', 'px.', 'go.', 'alt.Chart']
    ...
    def check_memory_patterns(self) -> Dict[str, Any]:
                ...
                for issue_type, pattern in patterns.items():
                    if re.search(pattern, source, re.MULTILINE):
                        memory_issues.append({
                            ...
                            'pattern': pattern,
                            'severity': 'high' if 'infinite' in issue_type else 'medium'
                        })
                ...
                if 'pd.read_' in source or 'np.load' in source:
                    if 'del ' not in source and cell_num < len(self.notebook.cells) - 1:
                        ...
                        if var_match:
                            ...
                            has_cleanup = False
                            for next_cell in self.notebook.cells[cell_num+1:cell_num+3]:
                                if next_cell.cell_type == 'code' and f'del {var_name}' in next_cell.source:
                                    has_cleanup = True
                                    break
                            if not has_cleanup:
                                self.info.append({
        ...
        return {
            'passed': len([i for i in memory_issues if i.get('severity') == 'high']) == 0,
            'issue_count': len(memory_issues),
            'high_severity': len([i for i in memory_issues if i.get('severity') == 'high']),
            'details': memory_issues
        }
    ...
    def check_code_quality(self) -> Dict[str, Any]:
        ...
        for cell_num, cell in enumerate(self.notebook.cells):
            if cell.cell_type == 'code':
                ...
                commented_code = len([l for l in lines if l.strip().startswith('#') and len(l.strip()) > 2])
        ...
        return {
            'passed': len([i for i in quality_issues if i.get('severity') == 'high']) == 0,
    ...
    def check_imports(self) -> Dict[str, Any]:
        ...
        for cell_num, cell in enumerate(self.notebook.cells):
            if cell.cell_type == 'code':
                ...
                # Find all imports
                imports = re.findall(r'^(import .+|from .+ import .+)$', source, re.MULTILINE)
                if imports:
        ...
        duplicates = set([x for x in import_statements if import_statements.count(x) > 1])
        if duplicates:
            import_issues.append({
    ...
    def check_markdown_quality(self) -> Dict[str, Any]:
        ...
        return {
            'passed': markdown_stats['total_cells'] > 0 and markdown_stats['empty_cells'] == 0,
...
class DataValidator:
    ...
    def _types_compatible(actual: str, expected: str) -> bool:
        ...
        for group in type_groups.values():
            if any(t in actual.lower() for t in group) and any(t in expected.lower() for t in group):
                return True
