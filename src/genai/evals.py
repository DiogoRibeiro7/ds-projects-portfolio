...
def _as_metric(name: str, per_item: list[float]) -> EvalMetricResult:
    ...
    return EvalMetricResult(
        name=name, mean=mean, ci_low=lo, ci_high=hi,
        n=len(per_item), per_item=per_item,
    )
...
def retrieval_metric_suite(preds: list[Prediction], records: list[EvalRecord],
                            k_list: tuple[int, ...] = (1, 3, 5, 10)) -> dict[str, EvalMetricResult]:
...
def faithfulness(preds: list[Prediction], judge: LLMClient) -> EvalMetricResult:
    ...
    for p in preds:
        ...
        v = _judge_call(
            judge, "judge.faithfulness",
            question=p.query, context=context, answer=p.answer.answer,
        )
...
def answer_relevance(preds: list[Prediction], judge: LLMClient) -> EvalMetricResult:
    ...
    for p in preds:
        v = _judge_call(
            judge, "judge.answer_relevance",
            question=p.query, answer=p.answer.answer,
        )
        per.append(float(v.get("score", 0.0)))
...
def context_precision(preds: list[Prediction], judge: LLMClient, k: int = 5) -> EvalMetricResult:
    ...
    for p in preds:
        ...
        for r in p.retrieved[:k]:
            v = _judge_call(judge, "judge.context_precision",
                             question=p.query, passage=r.text)
...
def judge_calibration(
    ...
    if len(judge_scores) < 3:
        return {"n": len(judge_scores), "pearson_r": 0.0, "spearman_rho": 0.0,
                "cohen_kappa": 0.0, "cohen_kappa_quadratic": 0.0,
    ...
    if np.ptp(judge_arr) == 0.0 or np.ptp(oracle_arr) == 0.0:
        return {"n": len(judge_scores), "pearson_r": 0.0, "spearman_rho": 0.0,
                "cohen_kappa": 0.0, "cohen_kappa_quadratic": 0.0,
                "degenerate": ("input is constant — correlation / kappa undefined. "
...
def build_eval_report(
    ...
    *,
    ...
    latency_summary = {
        f"{r}_p50_ms": float(np.median(v)) if v else 0.0 for r, v in latency_by_retriever.items()
    }
    latency_summary.update({
        f"{r}_p95_ms": float(np.quantile(v, 0.95)) if v else 0.0 for r, v in latency_by_retriever.items()
    })
    cost_summary = {
        f"{r}_total_usd": float(sum(v)) for r, v in cost_by_retriever.items()
    }
    ...
    if "answer_relevance" in generation:
        checks.append(("answer_relevance", generation["answer_relevance"].mean,
                        gate["min_answer_relevance"], ">="))
    checks.append(("p95_latency_ms", latency_summary.get(f"{primary}_p95_ms", 0.0),
    ...
    for name, observed, threshold, op in checks:
        ...
        if not passed:
            ok = False
        rationale.append(
            f"{'PASS' if passed else 'FAIL'}: {name}={observed:.3f} {op} {threshold:.3f}"
        )
