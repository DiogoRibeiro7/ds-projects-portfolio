...
class MLPipeline:
    ...
    def _generate_run_id(self) -> str:
        ...
        return hashlib.md5(f"{self.config.model_name}_{timestamp}".encode()).hexdigest()[:12]
    ...
    async def _run_tests(self, pipeline_run: Dict):
        ...
        try:
            ...
            with open(f"./workspace/{self.config.project_name}/coverage.json", 'r') as f:
            ...
            if coverage < self.config.test_coverage_threshold:
                raise ValueError(f"Test coverage {coverage:.2%} below threshold {self.config.test_coverage_threshold:.2%}")
        ...
        try:
            ...
            for metric_name, threshold in self.config.performance_thresholds.items():
                if metrics.get(metric_name, 0) < threshold:
                    raise ValueError(f"{metric_name} {metrics[metric_name]:.4f} below threshold {threshold}")
    ...
    async def _security_scan(self, pipeline_run: Dict):
        ...
        try:
            ...
            if result.stdout:
                secrets_found = [json.loads(line) for line in result.stdout.strip().split('\n')]
                if secrets_found:
            ...
            if vulnerabilities:
                logger.warning(f"Security vulnerabilities found: {len(vulnerabilities)}")
    ...
    async def _build_container(self, pipeline_run: Dict):
        ...
        try:
            ...
            # Push to registry
            for line in self.docker_client.images.push(image_tag, stream=True, decode=True):
                logger.debug(line)
...
"""
    ...
    async def _run_integration_test_suite(self) -> Dict:
        ...
        test_results['tests'].append({
            'name': 'prediction',
            'passed': response.status_code == 200 and 'prediction' in response.json()
        })
    ...
    async def _deploy_staging(self, pipeline_run: Dict):
        ...
        try:
            from production_deployment import ModelDeploymentManager, ModelDeploymentConfig
    ...
    async def _performance_tests(self, pipeline_run: Dict):
        ...
        try:
            ...
            if load_test_results['p95_latency'] > 1.0:  # 1 second
                raise ValueError(f"P95 latency {load_test_results['p95_latency']}s exceeds threshold")

            if load_test_results['error_rate'] > 0.01:  # 1% error rate
                raise ValueError(f"Error rate {load_test_results['error_rate']:.2%} exceeds threshold")
    ...
    async def _run_load_test(self, endpoint: str) -> Dict:
        ...
        # Run locust test
        result = subprocess.run(
            ['locust', '-f', 'load_test.py', '--host', endpoint,
        ...
        # Parse results
        with open(f"./workspace/{self.config.project_name}/tests/load_test_stats.csv", 'r') as f:
            import csv
        ...
        return {
            ...
            'error_rate': float(stats[0]['Failure Count']) / float(stats[0]['Request Count']),
    ...
    async def _manual_approval(self, pipeline_run: Dict):
        ...
        try:
            ...
            approval_request = {
                ...
                'model_name': self.config.model_name,
                'staging_metrics': pipeline_run['stages']['performance_tests']['performance_results'],
                'validation_metrics': pipeline_run['stages']['validation']['validation_results']
            }
    ...
    async def _send_slack_notification(self, pipeline_run: Dict, webhook_url: str):
        ...
        message = {
            ...
            "attachments": [{
                "color": "good" if pipeline_run['status'] == 'success' else "danger",
                "fields": [
                    {"title": "Model", "value": self.config.model_name, "short": True},
                    {"title": "Run ID", "value": pipeline_run['run_id'], "short": True},
                    {"title": "Duration", "value": self._calculate_duration(pipeline_run), "short": True},
                    {"title": "Trigger", "value": pipeline_run['trigger'].get('type', 'manual'), "short": True}
                ]
            }]
        }
