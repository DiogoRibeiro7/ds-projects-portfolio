...
class AnomalyGenerator:
    ...
    @staticmethod
    def inject_point_anomalies(data: np.ndarray,
    ...
    @staticmethod
    def inject_collective_anomalies(data: np.ndarray,
                                   n_segments: int = 3,
        ...
        for _ in range(n_segments):
            ...
            if pattern == 'shift':
                ...
                modified_data[start_idx:end_idx] += np.random.normal(0, magnitude * std, segment_length)
    ...
    @staticmethod
    def inject_contextual_anomalies(data: np.ndarray,
...
class StatisticalTests:
    ...
    def dixon_test(data: np.ndarray, alpha: float = 0.05) -> np.ndarray:
        ...
        if n <= 7:
            ...
            Q_low = (sorted_data[2] - sorted_data[0]) / (sorted_data[-2] - sorted_data[0])
...
class AnomalyFeatures:
    ...
    @staticmethod
    def create_anomaly_features(data: pd.Series, window_sizes: List[int] = [10, 30, 100]) -> pd.DataFrame:
        """
        ...
        for window in window_sizes:
            # Rolling statistics
            features[f'rolling_mean_{window}'] = data.rolling(window, center=True).mean()
            ...
            features[f'dist_from_mean_{window}'] = np.abs(data - features[f'rolling_mean_{window}'])
...
class AnomalyMetrics:
    ...
    @staticmethod
    def calculate_segment_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        """
...
class AdaptiveThreshold:
    ...
    def compute_threshold(self, scores: np.ndarray, contamination: float = 0.1) -> float:
        """
...
class AnomalyExplainer:
    ...
    @staticmethod
    def explain_anomaly(data_point: float, context: np.ndarray,
        ...
        explanation = {
            ...
            'z_score': (data_point - np.mean(context)) / np.std(context) if np.std(context) > 0 else 0,
            'percentile': stats.percentileofscore(context, data_point),
            'deviation': data_point - np.mean(context),
            'relative_deviation': (data_point - np.mean(context)) / np.mean(context) if np.mean(context) != 0 else 0
        }
        ...
        if features is not None:
            explanation['contributing_features'] = AnomalyExplainer._identify_contributing_features(features)
    ...
    def _identify_contributing_features(features: pd.DataFrame) -> List[str]:
        ...
        for col in features.columns:
            if 'zscore' in col or 'percentile' in col:
                value = features[col].iloc[0] if len(features) == 1 else features[col].mean()
